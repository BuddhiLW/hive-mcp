...
;; Helper: Check if hive-mcp.el is loaded
...
(def tools
  [{:name "mcp_memory_add"
    :description "Add an entry to project memory. Types: note, snippet, convention, decision. Optionally specify duration for TTL: ephemeral (1 day), short (7 days), medium (30 days), long (90 days), permanent (never expires). Requires hive-mcp.el."
   ...
   {:name "mcp_memory_query"
    :description "Query project memory by type with scope filtering. Returns stored notes, snippets, conventions, or decisions filtered by scope (auto-filters by current project + global unless specified). Requires hive-mcp.el."
    :inputSchema {:type "object"
                  :properties {"type" {:type "string"
   ...
   {:name "mcp_memory_query_metadata"
    :description "Query project memory by type, returning only metadata (id, type, preview, tags, created). Use this for efficient browsing - returns ~10x fewer tokens than full query. Follow up with mcp_memory_get_full to fetch specific entries. Requires hive-mcp.el."
    :inputSchema {:type "object"
                  :properties {"type" {:type "string"
                                       :enum ["note" "snippet" "convention" "decision" "conversation"]
                                       :description "Type of memory entries to query"}
   ...
   {:name "mcp_memory_get_full"
    :description "Get full content of a memory entry by ID. Use after mcp_memory_query_metadata to fetch specific entries when you need the full content. Requires hive-mcp.el."
    :inputSchema {:type "object"
