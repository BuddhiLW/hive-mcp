(ns hive-mcp.tools.compat-integration-test
  ...
  (:require [clojure.test :refer [deftest is testing use-fixtures]]
            ...
            [hive-mcp.tools.agent.helpers :as agent-helpers]
...
(defn isolated-state-fixture
  "Provide isolated DataScript + logic state for clean tests.
   Uses with-redefs to avoid mutating the live production connection
   (which is guarded by when-not-coordinator) and prevents elisp
   ling merging from polluting test state."
  [f]
  (let [test-conn (conn/create-conn)]
    (with-redefs [conn/get-conn (fn [] test-conn)
                  conn/ensure-conn (fn [] test-conn)
                  ;; Prevent elisp lings from polluting test state
                  agent-helpers/merge-with-elisp-lings identity]
  ...
  (testing "shims handle mix of string and keyword keys"
    (with-redefs [ec/eval-elisp-with-timeout (fn [_elisp _timeout]
      ...
      (let [result (compat/swarm-spawn-shim {"directory" "/mixed/path"
        ...
        (is (string? (:agent-id parsed))
            "agent-id should be present from spawn result")))))
