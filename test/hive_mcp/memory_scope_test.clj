(ns hive-mcp.memory-scope-test
  "Unit tests for hive-mcp-memory.el scope filtering functionality.
...
(def test-project-name "hive-mcp")
(def test-domain-name "development-tools")

(defn make-scope-tag-elisp
  "Generate elisp for hive-mcp-memory--make-scope-tag."
  ([level]
   (format "(progn (require 'hive-mcp-memory nil t) (hive-mcp-memory--make-scope-tag '%s))"
           (if (keyword? level) (clojure.core/name level) level)))
  ([level scope-name]
   (format "(progn (require 'hive-mcp-memory nil t) (hive-mcp-memory--make-scope-tag '%s %s))"
           (if (keyword? level) (clojure.core/name level) level)
           (pr-str scope-name))))

(defn inject-project-scope-elisp
  "Generate elisp for hive-mcp-memory--inject-project-scope."
  [tags]
  (format "(progn (require 'hive-mcp-memory nil t) (hive-mcp-memory--inject-project-scope '%s))"
          (pr-str tags)))

(defn applicable-scope-tags-elisp
  "Generate elisp for hive-mcp-memory--applicable-scope-tags."
  [& {:keys [project-name domain-name]}]
  (cond
    (and project-name domain-name)
    (format "(progn (require 'hive-mcp-memory nil t) (hive-mcp-memory--applicable-scope-tags %s %s))"
            (pr-str project-name) (pr-str domain-name))

    project-name
    (format "(progn (require 'hive-mcp-memory nil t) (hive-mcp-memory--applicable-scope-tags %s nil))"
            (pr-str project-name))

    domain-name
    (format "(progn (require 'hive-mcp-memory nil t) (hive-mcp-memory--applicable-scope-tags nil %s))"
            (pr-str domain-name))

    :else
    "(progn (require 'hive-mcp-memory nil t) (hive-mcp-memory--applicable-scope-tags))"))

(defn entry-matches-scope-elisp
  "Generate elisp for hive-mcp-memory--entry-matches-scope-p."
  [entry scope-tags]
  (format "(progn (require 'hive-mcp-memory nil t) (hive-mcp-memory--entry-matches-scope-p '%s '%s))"
          (pr-str entry) (pr-str scope-tags)))

(defn query-with-scope-elisp
  "Generate elisp for hive-mcp-api-memory-query with scope parameter."
  [type scope]
  (format "(json-encode (hive-mcp-api-memory-query %s nil 20 %s))"
          (pr-str type)
          (if scope (pr-str scope) "nil")))

;; =============================================================================
;; Test Scope Tag Creation - hive-mcp-memory--make-scope-tag
;; =============================================================================
...
(deftest test-make-scope-tag-global
  (testing "make-scope-tag creates 'scope:global' for global level"
    (let [result (make-scope-tag-elisp :global)]
      ...
      (is (str/includes? result "hive-mcp-memory--make-scope-tag"))
...
;; =============================================================================
;; Test Project Scope Injection - hive-mcp-memory--inject-project-scope
;; =============================================================================
...
(deftest test-inject-project-scope-adds-when-none-present
  (testing "inject-project-scope adds project scope when no scope tag present"
    (let [tags ["important" "todo"]
      ...
      (is (str/includes? result "hive-mcp-memory--inject-project-scope"))
...
;; =============================================================================
;; Test Applicable Scope Tags - hive-mcp-memory--applicable-scope-tags
;; =============================================================================
...
(deftest test-applicable-scope-tags-always-includes-global
  (testing "applicable-scope-tags always includes scope:global"
    (let [result (applicable-scope-tags-elisp)]
      (is (str/includes? result "hive-mcp-memory--applicable-scope-tags")))))
...
;; =============================================================================
;; Test Entry Scope Matching - hive-mcp-memory--entry-matches-scope-p
;; =============================================================================
...
(deftest test-entry-matches-scope-global-entry-matches-any-scope
  (testing "Entry without scope tag (global) matches any scope context"
    (let [entry {:id "entry1" :tags ["general" "note"]}
      ...
      (is (str/includes? result "hive-mcp-memory--entry-matches-scope-p")))))
...
(deftest test-entry-matches-scope-project-entry-matches-project-scope
  (testing "Entry with project scope matches same project scope"
    (let [entry {:id "entry1" :tags ["scope:project:hive-mcp" "feature"]}
          scope-tags ["scope:project:hive-mcp" "scope:global"]
          result (entry-matches-scope-elisp entry scope-tags)]
      (is (str/includes? result "hive-mcp-memory--entry-matches-scope-p"))
      (is (str/includes? result "scope:project:hive-mcp")))))
...
(deftest test-query-scope-filter-nil-auto-filters-current-project
  (testing "Query with nil scope auto-filters by current project + global"
    (let [result (query-with-scope-elisp "note" nil)]
      (is (str/includes? result "hive-mcp-api-memory-query"))
...
(deftest test-scope-system-workflow-create-and-query
  (testing "Complete workflow: create entry with scope, then query with scope filter"
    ...
    (let [query-elisp (query-with-scope-elisp "note" nil)]
      (is (str/includes? query-elisp "hive-mcp-api-memory-query")))))
...
(deftest test-scope-system-global-entry-visible-everywhere
  (testing "Global entry visible in all project contexts"
    ...
    (let [query-project-a (query-with-scope-elisp "convention" nil)]
      (is (str/includes? query-project-a "hive-mcp-api-memory-query")))))
