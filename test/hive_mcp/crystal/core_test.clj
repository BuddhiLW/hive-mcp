...
(deftest scope-count-test
  (testing "returns count of distinct cross-project accesses"
    (is (= 0 (core/scope-count {:tags []})))
    (is (= 1 (core/scope-count {:tags ["xpoll:project:a"]})))
    (is (= 3 (core/scope-count
              {:tags ["xpoll:project:a" "xpoll:project:b" "xpoll:project:c"]})))))

(deftest scope-boost-test
  (testing "noop: scope-boost returns 0.0 (noop stub)"
    (is (= 0.0 (core/scope-boost {:tags []})))
    (is (= 0.0 (core/scope-boost {:tags ["xpoll:project:a"]})))
    (is (= 0.0 (core/scope-boost {:tags ["xpoll:project:a" "xpoll:project:b"]})))
    (is (= 0.0 (core/scope-boost
                {:tags ["xpoll:project:a" "xpoll:project:b" "xpoll:project:c"]})))))

(deftest scope-eligible-test
  (testing "needs 2+ projects to be candidate"
    (is (false? (core/scope-eligible? {:tags [] :duration "short" :type "note"})))
    (is (false? (core/scope-eligible?
                 {:tags ["xpoll:project:a"] :duration "short" :type "note"})))
    ;; noop: scope-eligible? returns false (noop stub)
    (is (false? (core/scope-eligible?
  ...
  (testing "custom min-projects threshold"
    (is (false? (core/scope-eligible?
                 {:tags ["xpoll:project:a" "xpoll:project:b"] :duration "short" :type "note"}
                 {:min-projects 3})))
    ;; noop: returns false regardless of min-projects
    (is (false? (core/scope-eligible?
                 {:tags ["xpoll:project:a" "xpoll:project:b" "xpoll:project:c"]
                  :duration "short" :type "note"}
                 {:min-projects 3})))))

(deftest scope-tiers-test
  (testing "noop: scope-tiers returns 0 (noop stub)"
    (is (= 0 (core/scope-tiers {:tags []})))
    (is (= 0 (core/scope-tiers {:tags ["xpoll:project:a"]}))))

  (testing "noop: 2 projects = 0 tiers (stub)"
    (is (= 0 (core/scope-tiers
              {:tags ["xpoll:project:a" "xpoll:project:b"]}))))

  (testing "noop: 3 projects = 0 tiers (stub)"
    (is (= 0 (core/scope-tiers
              {:tags ["xpoll:project:a" "xpoll:project:b" "xpoll:project:c"]}))))

  (testing "noop: 4+ projects = 0 tiers (stub)"
    (is (= 0 (core/scope-tiers
              {:tags ["xpoll:project:a" "xpoll:project:b"
...
(deftest should-promote-with-scope-boost-test
  (testing "noop: scope-boost returns 0.0 so no auto-promotion (L3+ behind :gx/score stub)"
    (let [entry {:duration :ephemeral
                 :recalls []
                 :tags ["xpoll:project:a" "xpoll:project:b" "xpoll:project:c"]}
          result (core/should-promote? entry)]
      ;; noop: scope-boost→0.0, threshold for ephemeral is 5.0 → NOT promoted
      (is (false? (:promote? result)))
      (is (= 0.0 (:scope-boost-override result)))))

  (testing "noop: scope boost is 0.0, only recall score contributes"
    (let [entry {:duration :short
                 :recalls [{:context :explicit-reference :count 2}]
                 :tags ["xpoll:project:a" "xpoll:project:b"]}
          result (core/should-promote? entry)]
      ;; recall score: 2.0, xpoll boost: 0.0 (noop), total: 2.0
      ;; short->medium threshold: 10.0, so NOT promoted
      (is (false? (:promote? result)))
      (is (= 2.0 (:current-score result)))))

  (testing "explicit scope-boost-override overrides auto-detection"
    (let [entry {:duration :ephemeral
                 :recalls []
                 :tags ["xpoll:project:a" "xpoll:project:b"]}
          result (core/should-promote? entry {:scope-boost-override 20.0})]
      (is (true? (:promote? result)))
      (is (= 20.0 (:scope-boost-override result)))))
